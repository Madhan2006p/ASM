...
def run_findomain(target):
    ...
    with temporary_file(suffix=".txt") as output_file:
    ...
    raw_output = command_output or combine_output(
        execution["stdout"],
        execution["stderr"],
    )
    parsed_subdomains = parse_findomain(command_output or execution["stdout"])
    parsed_output = {
