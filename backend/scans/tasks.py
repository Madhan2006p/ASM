...
def run_nuclei_vuln_scan(self, scan_id):
    """
    Executes Nuclei scanner templates and processes CVE/vulnerability findings
    """
    scan = Scan.objects.get(id=scan_id)
    scan.status = 'RUNNING'
    scan.celery_task_id = self.request.id
    ...
    # Run Nuclei locally with CVE, misconfig, and exposure tags as specified
    command = [
    ...
    try:
        ...
        if not output_file.exists():
            # Mock output generation for demonstration if Nuclei binary is missing
            mock_data = [
                {
                    "info": {
                        "name": "Nginx Default Page Exposed",
                        "severity": "info",
                        "description": "The default Nginx page is exposed.",
                        "remediation": "Remove the default configuration."
                    }
                },
                {
                    "info": {
                        "name": "OpenSSH 8.2p1 User Enumeration",
                        "severity": "medium",
                        "description": "OpenSSH is vulnerable to user enumeration.",
                        "remediation": "Upgrade OpenSSH to the latest version.",
                        "classification": {"cve-id": "CVE-2020-15778"}
                    }
                }
            ]
        ...
        if output_file.exists():
            with open(output_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    # Extract vulnerability properties
                    ...
                    # Enrich CVSS from NVD API if CVE is found but score is missing
                    cvss_score = None
                    if cve_id:
                        cvss_score = fetch_nvd_cvss_score(cve_id) or 7.5 # Mock score if API fails
