...
class FuzzingResultViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return FuzzingResult.objects.filter(endpoint__target__user=self.request.user)
...
class FuzzingQueueViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return FuzzingQueue.objects.filter(endpoint__target__user=self.request.user)
